...
async fn non_stream_translation() {
    ...
    let engine = OpenAiEngine::new(OpenAiConfig {
        id: "mock".into(), base_url: server.uri(), api_key: "k".into(),
        model: "m".into(), system_prompt: None, user_prompt_template: None,
    });
...
async fn stream_deltas_concatenated() {
    ...
    let full = engine.translate_stream(&req(), |d| deltas.push(d.to_string())).await.unwrap();
    assert_eq!(full, "你好");
...
async fn api_error_maps_to_engine_error() {
    ...
    assert!(matches!(err, pardon_core::engine::EngineError::Api(_)), "got: {err:?}");
}
